...
class SpecialistServicesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_services_especialista, container, false)
    ...
    private fun checkAuthorizationAndRedirect(serviceName: String) {
        firestore.collection("especialistas").document(uid).get().addOnSuccessListener { document ->
            if (document.exists()) {
                ...
                if (autorizado) {
                    val intent = Intent(activity, SpecialistCreateServices::class.java)
    ...
    companion object {
        ...
        fun newInstance() = SpecialistServicesFragment() // Devuelve una nueva instancia del fragmento
    }
}
